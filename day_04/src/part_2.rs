...
fn main() {
    ...
    for n in input {
        ...
        let (mut finished, unfinished): (Vec<Board>, Vec<Board>) = boards.into_iter()
            .partition(|b| b.is_filled());
        winners.append(&mut finished);

        if unfinished.len() == 0 {
            println!("{}", n * winners[winners.len()-1].unmarked_sum());
            return;
        }
        boards = unfinished;
    }
